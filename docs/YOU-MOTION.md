...
未出手的失败（`onInvalid`）同一套 0 格砸入，不进 `playDir`、不加 `busy`。
...
| 吃到收集星 | 滑到该格**当下**起飞：自身格正上浮 **100px**；金黄泛光在上升段（110ms）Alpha 淡到 0 → 蹲 50px → 飞 HUD 槽 0 / 1 |
| 滑到门格 | **当下**终点星同样起飞 → HUD 槽 2 |
| 结算 | 等**全部飞星结束** + HUD 弹出（约 280ms）再出 overlay |

HUD 三槽。评价公式仍是规则里的 **1 + 吃到的星**，见 ICE-PUZZLE。  
结算卡：全屏蒙黑 + 奶油面板 + 同款三星 + 圆形重开 / 绿色下一关。弹出：蒙黑 280ms 淡入，卡片 380ms 从下轻移放大落到 1（ease-out）。形态见 [VISUAL.md](./VISUAL.md) §5。
...
关卡提示默认隐藏。**只有第 1 关**出提示，其余关关掉。进关约 **0.7s** 后从屏幕下方滑入；成功滑一步后**原地淡出**。未完成该步再停手约 2.5s 会再滑入。  
第 1 关三步：①「向右滑动撞击木箱。」→ 撞停后换 ②「向右滑动推动木箱」→ 推完换 ③「将角色滑动到红色目标位置」。步与步之间约 420ms 再滑入。
...
| `HINT_FIRST_MS` / `IDLE` / `NEXT` | 700 / 2500 / 420 |
